...
|---|---|---|---|
| arXiv 2607.20093, *Retail Trader's Ruin* (Darmanin) | BLOCKED / SNIPPET | **Medium-High methodology, but NOT independent** | Pre-declared gates, Benjamini–Yekutieli, stationary bootstrap, equivalence tests. **Author's firm (Hecatus Research) sells trading signals and algo-trading services** — commercially benefits from debunking free retail signals. Snippet claim of included replication code is **uncorroborated** — see below. |
| `github.com/adamd1985/quant_research` | **CLONED** (only source read at origin) | Medium | Author's general quant repo, HEAD `64ee28d` 2026-07-01. Does **not** contain the paper's replication code (0 hits for `retail trader`, `benjamini`/`yekutieli`, `equivalence test`, `sell.in.may`). Does contain `papers/The Stationary Bootstrap.pdf` and `papers/A_Test_for_Superior_Predictive_Ability.pdf`, corroborating that the author works with the toolkit the abstract describes. |
